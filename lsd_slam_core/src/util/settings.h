...
#define VAL_SUM_MIN_FOR_CREATE                                                                                         \
  (30)  // minimal summed validity over 5x5 region to create a new hypothesis for non-blacklisted pixel (hole-filling)
...
#define MAX_VAR                                                                                                        \
  (0.5f * 0.5f)  // initial variance on creation - if variance becomes larter than this, hypothesis is removed.
...
#define MAX_ERROR_STEREO                                                                                               \
  (1300.0f)  // maximal photometric error for stereo to be successful (sum over 5 squared intensity differences)
#define MIN_DISTANCE_ERROR_STEREO                                                                                      \
  (1.5f)  // minimal multiplicative difference to second-best match to not be considered ambiguous.
...
#define MIN_ABS_GRAD_CREATE (isDisplacement? minUseDispGrad: minUseGrad)
#define MIN_ABS_GRAD_DECREASE (isDisplacement? minUseDispGrad: minUseGrad)
