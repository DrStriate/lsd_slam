...
Sim3Tracker::Sim3Tracker(int w, int h, Eigen::Matrix3f K)
{
  ...
  lastResidual = 0;
  iterationNumber = 0;
  lastDepthResidual = lastPhotometricResidual = lastDepthResidualUnweighted = lastPhotometricResidualUnweighted =
      lastResidualUnweighted = 0;
  pointUsage = 0;
}
                                 ...
                                 const Sim3& frameToReference_initialEstimate, int startLevel, int finalLevel)
{
  ...
  for (int lvl = startLevel; lvl >= finalLevel; lvl--)
  {
    ...
    for (int iteration = 0; iteration < settings.maxItsPerLvl[lvl]; iteration++)
    {
      ...
      while (true)
      {
        ...
        if (buf_warped_size < 0.5 * MIN_GOODPERALL_PIXEL_ABSMIN * (width >> lvl) * (height >> lvl) ||
            buf_warped_size < 10)
        {
          diverged = true;
          return Sim3();
        ...
        if (error.mean < lastErr.mean)
        {
          ...
          if (enablePrintDebugInfo && printTrackingIterationInfo)
          {
            ...
            printf("(%d-%d): ACCEPTED increment of %f with lambda %.1f, residual: %f -> %f\n", lvl, iteration,
                   sqrt(inc.dot(inc)), LM_lambda, lastErr.mean, error.mean);

            printf("         p=%.4f %.4f %.4f %.4f %.4f %.4f %.4f\n", referenceToFrame.log()[0],
                   referenceToFrame.log()[1], referenceToFrame.log()[2], referenceToFrame.log()[3],
                   referenceToFrame.log()[4], referenceToFrame.log()[5], referenceToFrame.log()[6]);
          }
  ...
  if (enablePrintDebugInfo && printTrackingIterationInfo)
  {
    ...
    printf("pOld = %.5f %.5f %.5f %.5f %.5f %.5f %.5f\n", frameToReference_initialEstimate.inverse().log()[0],
           frameToReference_initialEstimate.inverse().log()[1], frameToReference_initialEstimate.inverse().log()[2],
           frameToReference_initialEstimate.inverse().log()[3], frameToReference_initialEstimate.inverse().log()[4],
           frameToReference_initialEstimate.inverse().log()[5], frameToReference_initialEstimate.inverse().log()[6]);
    printf("pNew = %.5f %.5f %.5f %.5f %.5f %.5f %.5f\n", referenceToFrame.log()[0], referenceToFrame.log()[1],
           referenceToFrame.log()[2], referenceToFrame.log()[3], referenceToFrame.log()[4], referenceToFrame.log()[5],
...
#if defined(ENABLE_SSE)
void Sim3Tracker::calcSim3BuffersSSE(const TrackingReference* reference, Frame* frame, const Sim3& referenceToFrame,
...
#if defined(ENABLE_NEON)
void Sim3Tracker::calcSim3BuffersNEON(const TrackingReference* reference, Frame* frame, const Sim3& referenceToFrame,
                                      int level, bool plotWeights)
{
  calcSim3Buffers(reference, frame, referenceToFrame, level, plotWeights);
}
  ...
  for (; refPoint < refPoint_max; refPoint++, refGrad++, refColVar++)
  {
    ...
    *(buf_d + idx) = 1.0f / (*refPoint)[2];
    ...
    if (plotSim3TrackingIterationInfo)
    {
      ...
      {
        setPixelInCvMat(&debugImageDepthResiduals, getGrayCvPixel(128 + 800 * *(buf_residual_d + idx)), x, y,
                        (width / w));

        if (plotWeights)
        {
          setPixelInCvMat(&debugImageWeightD, getGrayCvPixel(255 * (1 / 60.0f) * sqrtf(*(buf_weight_VarD + idx))), x, y,
                          (width / w));
          setPixelInCvMat(
              &debugImageWeightedResD,
              getGrayCvPixel(128 + (128 / 5.0f) * sqrtf(*(buf_weight_VarD + idx)) * *(buf_residual_d + idx)), x, y,
              (width / w));
        }
      }
      ...
      if (plotWeights)
      {
        setPixelInCvMat(&debugImageWeightP, getGrayCvPixel(255 * 4 * sqrtf(*(buf_weight_VarP + idx))), x, y,
                        (width / w));
        setPixelInCvMat(&debugImageHuberWeight, getGrayCvPixel(255 * *(buf_weight_Huber + idx)), x, y, (width / w));
        setPixelInCvMat(
            &debugImageWeightedResP,
            getGrayCvPixel(128 + (128 / 5.0f) * sqrtf(*(buf_weight_VarP + idx)) * *(buf_warped_residual + idx)), x, y,
            (width / w));
      }
    }
...
#if defined(ENABLE_SSE)
Sim3ResidualStruct Sim3Tracker::calcSim3WeightsAndResidualSSE(const Sim3& referenceToFrame)
{
  ...
  for (int i = 0; i < buf_warped_size - 3; i += 4)
  {
    ...
    __m128 w_ds = _mm_rcp_ps(
        _mm_add_ps(_mm_load_ps(buf_warped_idepthVar + i),
                   _mm_mul_ps(g2s, _mm_mul_ps(g2s, _mm_mul_ps(depthVarFacs, _mm_load_ps(buf_idepthVar + i))))));
    ...
    __m128 weighted_rds = _mm_mul_ps(_mm_load_ps(buf_residual_d + i), _mm_sqrt_ps(w_ds));
    weighted_rds = _mm_max_ps(weighted_rds, _mm_sub_ps(zeros, weighted_rds));

    // depthValid = sv > 0
    __m128 depthValid = _mm_cmplt_ps(zeros, _mm_load_ps(buf_warped_idepthVar + i));  // bitmask 0xFFFFFFFF for idepth
                                                                                     // valid, 0x000000 otherwise
    ...
    whs = _mm_or_ps(_mm_and_ps(whs, ones), _mm_andnot_ps(whs, _mm_mul_ps(huber_ress, _mm_rcp_ps(weighted_abs_ress))));
    ...
    if (i + 3 < buf_warped_size)
    {
      ...
      sumResD = _mm_add_ps(sumResD, _mm_and_ps(depthValid, _mm_mul_ps(whs, _mm_mul_ps(weighted_rds, weighted_rds))));
...
Sim3ResidualStruct Sim3Tracker::calcSim3WeightsAndResidual(const Sim3& referenceToFrame)
{
  ...
  if (plotSim3TrackingIterationInfo)
  {
    printf("rd %f, rp %f, wrd %f, wrp %f, wd %f, wp %f\n ", sum_rd / sum_num_d, sum_rp / sum_num_p, sum_wrd / sum_num_d,
           sum_wrp / sum_num_p, sum_wd / sum_num_d, sum_wp / sum_num_p);
  }
  return sumRes;
}
